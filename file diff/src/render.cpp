...
void WindowClass::DrawSelections()
{
...
void WindowClass::DrawInputText()
{
    static char buffer1[50000] = "";
    static char buffer2[50000] = "";

    ImGui::BeginChild("Editor1", ImVec2(600.0F, -1.0f));
    if (ImGui::InputTextMultiline("##Editor_",
                                  buffer1,
                                  IM_ARRAYSIZE(buffer1),
                                  ImVec2(1200.0F, -1.0f),
                                  ImGuiInputTextFlags_None))
    {
        textContent1 = std::string(buffer1);
    }
    ImGui::EndChild();

    ImGui::SameLine();

    ImGui::BeginChild("Editor2", ImVec2(600.0F, -1.0f));
    if (ImGui::InputTextMultiline("##Editor_",
                                  buffer2,
                                  IM_ARRAYSIZE(buffer2),
                                  ImVec2(1200.0F, -1.0f),
                                  ImGuiInputTextFlags_None))
    {
        textContent2 = std::string(buffer2);
    }
    ImGui::EndChild();
}
...
{
    constexpr static auto swap_widgth = 40.0F;
    const auto parent_size = ImVec2(ImGui::GetContentRegionAvail().x, 500.0F);
    const auto child_size =
        ImVec2(parent_size.x / 2.0F - swap_widgth, parent_size.y);
    ...
    if (ImGui::BeginChild("Swap", ImVec2(swap_widgth, child_size.y), true))
    {
